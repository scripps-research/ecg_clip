...
from utils import clip_loss, make_optimizer_and_scheduler, random_obfuscate, recon_loss, tokenize_ecg
...
def make_collate_fn(tokenizer):
    def collate(batch):
        ...
        return {"ecg": ecgs, "input_ids": enc["input_ids"], "attention_mask": enc["attention_mask"]}
    return collate


def run_epoch(model, loader, device, replace, lmbda, optimizer=None, scheduler=None):
    ...
    with torch.set_grad_enabled(is_train):
        for batch in tqdm(loader, leave=False):
            ...
            obf_ecg, clean_idx, obf_idx = random_obfuscate(ecg, replace)

            ecg_emb, text_emb, recon = model.forward_clip(
                obf_ecg, tokens, attn_mask, clean_idx, obf_idx, decode=True
            )
            loss = (lmbda * recon_loss(recon, ecg, obf_idx) +
                    (1. - lmbda) * clip_loss(ecg_emb, text_emb, model.log_scale))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ECG-CLIP stage 2: contrastive + reconstruction pretraining")
    ...
    parser.add_argument("--replace",       type=float, default=0.75)
    ...
    for epoch in range(1, args.max_epochs + 1):
        train_loss = run_epoch(model, train_loader, device, args.replace, args.lmbda, optimizer, scheduler)
        val_loss   = run_epoch(model, val_loader,   device, args.replace, args.lmbda)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                ...
                f"{args.output_dir}/stage2_best.pt",
            )
